...
#ifndef RTLIB_CORE_STM32F1_LIBUSBHOST_USBH_DEVICE_DRIVER_H_
#define RTLIB_CORE_STM32F1_LIBUSBHOST_USBH_DEVICE_DRIVER_H_
...
/**
 * @brief Types of transfers supported by endpoints.
 */
...
/**
 * @brief Enumeration of USB transfer rates.
 */
enum USBH_SPEED {
  /**
   * @brief USB Full-Speed, i.e. 12 Mbps.
   */
  USBH_SPEED_FULL = 0,
  /**
   * @brief USB Low-Speed, i.e. 1.5 Mbps.
   */
  USBH_SPEED_LOW = 1,
  /**
   * @brief USB High-Speed, i.e. 480 Mbps.
   */
  USBH_SPEED_HIGH = 2,
};
...
enum USBH_PACKET_CALLBACK_STATUS {
  ...
  USBH_PACKET_CALLBACK_STATUS_ESIZE = 1,
...
struct _usbh_packet {
  ...
  /**
   * @brief callback this will be called when the packet is finished - either successfully or not.
   */
  usbh_packet_callback_t callback;
...
#define ERROR(arg) ToUART("ERROR: %s() - %d: file: %s, line: %d\r\n", __func__, arg, __FILE__, __LINE__)
...
#endif  // RTLIB_CORE_STM32F1_LIBUSBHOST_USBH_DEVICE_DRIVER_H_
